class OrdersController < ApplicationController
  ...
  def show
    ...
    if(@order.payment_status == 'PAID')
      if(@order.email_status != 'SENT')
        ...
        @order.update(email_status: 'SENT')
      end
    end
  end
  ...
  def create
    ...
    if @order.save
      ...
      UserMailer.order_request_mail(@order.email, @menu.id, @order.id, params[:locale]).deliver_now!
      UserMailer.order_request_mail("raul@losibaritas.com", @menu.id, @order.id, params[:locale]).deliver_now!
  ...
  def order_params
      params.require(:order).permit(
          ...
          :net_price, :vat_amount, :currency, :cook_date, :cook_time, :cook_location, :order_type, :type, :event_id, :payment_status, :email_status)
  end
end
